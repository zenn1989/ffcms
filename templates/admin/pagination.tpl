        ...
        <ul class="pagination pagination-centered">
            {% if local.total/local.count > 10 %} {# more then 10 items available #}
                {% if local.index < 4 %} {# list start #}
                    {% for pageidx in 0..4 %}
                        {{ listmacro.li_pagination(system.file_name, local.link, pageidx, pageidx == local.index) }}
                    {% endfor %}
                ...
                {% else %} {# list middle +-4 items from this +1st + last#}
                    ...
                    {% endfor %}
                    <li class="disabled"><a href="#">...</a></li> {# splitter '...' #}
                    {{ listmacro.li_pagination(system.file_name, local.link, local.lastpage, false) }} {# last page #}
                {% endif %}
